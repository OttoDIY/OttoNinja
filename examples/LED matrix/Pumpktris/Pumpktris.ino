...
Adafruit_8x16matrix matrixBottom = Adafruit_8x16matrix();
...
//  matrixTop.setTextColor(LED_ON);
  ...
  matrixBottom.clear();
  matrixBottom.writeDisplay();
  matrixBottom.setTextSize(1);
  matrixBottom.setTextWrap(false);  // we don't want text to wrap, so it scrolls nicely
  matrixBottom.setTextColor(LED_ON);
  ...
  for (int8_t x=8; x>= -(theLevelLength); x--) {       // start offscreen and scroll the level from right to left
    ...
    for (int8_t x=8; x>= -(theScoreLength); x--) {     // start offscreen and scroll the score from right to left
    matrixBottom.clear();
    matrixBottom.setCursor(x,0);
    matrixBottom.print(theScore);
    matrixBottom.writeDisplay();
    delay(50);
  }
...
{   
 ...
 Serial.begin(9600); 
  ...
  matrixBottom.begin(0x70);          // initialize the bottom matrix
  
  matrixTop.setRotation(4);          // set the rotation of the top matrix
  matrixBottom.setRotation(2);       // set the rotation of the bottom matrix
                                     // you may need to change these rotation values depending on the way your matrix is plugged into the shield
  
  matrixTop.setBrightness(1);        // set the brightness of the top matrix
  matrixBottom.setBrightness(1);     // set the brightness of the bottom matrix
                                     // acceptable values are 0-16
                                     // I set to 1 because that's what looked best and had the least bleed-through to other "pixels" on the pumpkin
...
{  
  for (byte i = 0; i < NUMBUTTONS; i++) {
    ...
    matrixBottom.writeDisplay();  // update the bottom LEDs with the new info
