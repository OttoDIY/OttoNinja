...
#include "RemoteXYServer.h" // Include CRemoteXYServer header file
#include "RemoteXYClient.h" // Include CRemoteXYClient header file
...
class CRemoteXYConnectionServer: public CRemoteXYConnectionComm, public CRemoteXYClientAvailableListener { 
  public:
    ...
    CRemoteXYConnectionServer (CRemoteXYComm * _comm, const uint16_t _port = 0) : CRemoteXYConnectionComm (_comm) {
      port = _port;
      clients = NULL;
    ...
    // Destructor to free up memory allocated for clients and wires
    ~CRemoteXYConnectionServer() {
      CRemoteXYClient * client = clients;
      while (client) {
        CRemoteXYClient * nextClient = client->next;
        delete client;
        client = nextClient;
      }
      ...
      while (wire) {
        CRemoteXYWireStream * nextWire = wire->next;
        delete wire;
        wire = nextWire;
      }
    }
    ...
    void clientAvailable () override {  
      ...
      if (!client) {   
        ...
        if (!client) {
          // Debug logging for client allocation failure
#if defined(REMOTEXY__DEBUGLOG)
          RemoteXYDebugLog.write ("Client was not allocated");
#endif
          return;
        }
        client->next = clients;
      ...
      if (server->available (client)) {
        ...
        if (CRemoteXYThread::runningCount (data) < REMOTEXY_MAX_CLIENTS) { 
          ...
          if (!wire) {
            ...
            if (!wire) {
              // Debug logging for wire allocation failure
#if defined(REMOTEXY__DEBUGLOG)
              RemoteXYDebugLog.write ("Wire was not allocated");
#endif
              return;
            }
            wire->next = wires;
